...
class ExtractEpubService
{
    ...
    public function chooseTitle($findTitle, $pathfile){
        if($findTitle !== ''){
            return $findTitle;
        }; 
        $string_path = explode('/',$pathfile);
        $first_element = explode('.epub',end($string_path));
        return array_shift($first_element);
    }
}
